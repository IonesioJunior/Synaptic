...
func TestDeliverMessage(t *testing.T) {
	...
	// Create server
	server := NewServer(db, authService, 10.0, 20, nil)
	...
	t.Run("Broadcast Message Delivery", func(t *testing.T) {
		...
		case receivedMsg := <-client2.send:
			...
			if decoded.Header.From != "user1" || decoded.Body.Content != "Hello everyone!" {
	...
	t.Run("Direct Message Delivery", func(t *testing.T) {
		...
		case receivedMsg := <-client2.send:
			...
			if decoded.Header.From != "user1" || decoded.Header.To != "user2" || decoded.Body.Content != "Hello user2!" {
				t.Errorf("Message content mismatch: got %+v", decoded)
			}
		case <-time.After(time.Second):
...
func TestRetrieveUndeliveredMessages(t *testing.T) {
	...
	// First expect the query for user's created_at timestamp
	createdAt := time.Now().Add(-time.Hour)
	mock.ExpectQuery(`SELECT created_at FROM users WHERE user_id = \?`).
		WithArgs("user1").
...
func TestHandleWebSocket(t *testing.T) {
	...
	t.Run("No Token", func(t *testing.T) {
		...
		if !strings.Contains(rr.Body.String(), "Missing authentication token") {
			t.Errorf("Expected missing authentication token message, got: %s", rr.Body.String())
		}
	})
