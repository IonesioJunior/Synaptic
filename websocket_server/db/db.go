...
func RunMigrations(db *sql.DB) error {
	...
	if _, err := db.Exec(messageDeliveries); err != nil {
		return fmt.Errorf("failed to create broadcast_deliveries table: %v", err)
	}
	...
	// Add X25519 public key column if it doesn't exist (for existing databases)
	alterTable := `ALTER TABLE users ADD COLUMN x25519_public_key TEXT;`
