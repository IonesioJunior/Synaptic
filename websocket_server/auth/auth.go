...
func NewService(db *sql.DB, securityLogFile string) *Service {
	...
	return &Service{
		db:        db,
		jwtSecret: secret,
		logger:    NewLogger(securityLogFile),
	}
}
...
type RegistrationPayload struct {
	...
	X25519PublicKey string `json:"x25519_public_key,omitempty"` // base64-encoded X25519 public key for encryption
}
...
func (a *Service) HandleRegistration(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		a.logger.LogAuthEvent(SecurityEvent{
			...
			IP:        clientIP,
			Success:   false,
			Details:   fmt.Sprintf("Username: %s, Error: %v", payload.Username, err),
		})
	...
	a.logger.LogAuthEvent(SecurityEvent{
		...
		IP:        clientIP,
		Success:   true,
		Details:   fmt.Sprintf("Username: %s", payload.Username),
	})
...
func (a *Service) HandleGetUserInfo(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT user_id, username, public_key, COALESCE(x25519_public_key, '') FROM users WHERE user_id = ?"
	var user struct {
		UserID          string `json:"user_id"`
		Username        string `json:"username"`
...
func (a *Service) handleChallengeResponse(w http.ResponseWriter, r *http.Request) {
	...
	// Decode the challenge from base64 to get the original bytes
	challengeBytes, err := base64.StdEncoding.DecodeString(challenge)
	if err != nil {
		http.Error(w, "Invalid challenge encoding", http.StatusInternalServerError)
		return
	}
	...
	a.logger.LogAuthEvent(SecurityEvent{
		...
		IP:        clientIP,
		Success:   true,
		Details:   "Login successful, JWT issued",
	})
