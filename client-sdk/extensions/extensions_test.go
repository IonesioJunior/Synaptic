...
func TestMessageRouter_HandleMessage(t *testing.T) {
	...
	testMsg := &TestMessage{BaseExtendedMessage: &types.BaseExtendedMessage{}}
	factory.RegisterType("TestMessage", testMsg)
	...
	baseMsg := &types.Message{
		...
		Body: types.MessageBody{
			Content: `{"type": "TestMessage", "test_data": "routing test"}`,
		},
	}
	...
	if handler.handled[0] != "TestMessage" {
		t.Errorf("Expected message type TestMessage, got %s", handler.handled[0])
	}
}
...
func TestMessageRouter_HandlerError(t *testing.T) {
	...
	baseMsg := &types.Message{
		...
		Body:   types.MessageBody{Content: `{"type": "TestMessage", "test_data": "error test"}`},
	}
...
func TestAsyncHandler_QueueFull(t *testing.T) {
	...
	// Create async handler with tiny queue
	...
	// These should succeed (queue size 2 + 1 being processed)
	for i := 0; i < 3; i++ {
		err := async.HandleMessage(msg)
		if err != nil {
			t.Errorf("Message %d should have been queued: %v", i, err)
		}
	}
...
func TestDefaultMessageFactory_ConcurrentRegister(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			msgType := &TestMessage{BaseExtendedMessage: &types.BaseExtendedMessage{}}
